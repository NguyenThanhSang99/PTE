...
class READ():
    def __init__(self, window_size, path="data"):
        self.path = path
    ...
    def generate_graphs(self):
        ...
        for x in self.d:
            ...
            label = self.l[x[1]]
            for y in x[0]:
                # word is vertex id of "word and document" graph
                word = self.w[y]
                left = int(index-self.window_size/2)
                right = int(index+self.window_size/2)
                if left < 0:
                    left = 0
                if right > len1:
                    right = len1-1
    ...
    def gen_edgeprob(self):
        ...
        for doc in self.w2d.keys():
            for word in self.w2d[doc].keys():
                # frequency of that word in the doc
                pd.append(self.w2d[doc][word])
                v3.append(doc)
                v4.append(word)
        ...
        for label in self.w2l.keys():
            for word in self.w2l[label].keys():
                # frequency of that word in that label
                pl.append(self.w2l[label][word])
                v5.append(label)
                v6.append(word)
