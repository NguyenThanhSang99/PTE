...
class train_pte(object):
    ...
    def train(self):
        ...
        pte = PTE(self.graphs.nvertex, self.ndims,
                  self.graphs.ndocs, self.graphs.nlabel)
        pte.ww_model()
        pte.wd_model()
        pte.wl_model()
        ...
        for it in range(0, self.graphs.nedge, self.batch_size):
            ...
            while k < sample.shape[0]:
                i = v1[sample[k]]-1
                j = v2[sample[k]]-1
                i_set = np.asarray(np.random.choice(
                    self.graphs.nvertex, size=self.k, p=nP), dtype=np.int32)
                if i in i_set:
